...
dump = async(mod) => {
  var db, fp, q, ref, schema_name, uri, x;
  uri = 'postgres://' + mod;
  ...
  for (x of ref) {
    ...
    await Promise.all([$`${ROOT}/pg/table.sh ${db} ${schema_name}`, $`${ROOT}/pg/data.sh ${bucket} ${db} ${schema_name}`]);
    fp = `pg/table/${db}/${schema_name}.sql`;
    write(fp, read(fp).replaceAll('CREATE SCHEMA ', 'CREATE SCHEMA IF NOT EXISTS '));
  }
};
